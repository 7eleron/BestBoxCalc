...
def result_data_casket(a, b, c, cardboard_req, paper_req, kol, cur_euro, lam_req):
    if kol >= 500 and a >= 80 and b >= 65 and c >= 10:
        # расчет изготовления лотка на автоматической машине
        if a <= 460 and b <= 380 and c <= 120:
            result = result_data_casket_auto(a, b, c, cardboard_req, paper_req, cur_euro, kol, laminating=lam_req)
            # расход материала
        ...
        # расчет изготовления лотка вручную
            ...
            result = result_data_casket_hand(a, b, c, cardboard_req, paper_req, cur_euro, kol, laminating=lam_req)
            # расход материала
            result_cardboard = result.get('Расход картона')
            result_paper = result.get('Расход бумаги')
            # стоимость работы
